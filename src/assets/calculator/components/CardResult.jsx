...
const CardResult = (dataSave) => {
  ...
  useEffect(() => {
    if (dataSave) {
      ...
      let percent =
        (Number(dataSave.data.expense) / Number(dataSave.data.income)) * 100;
    ...
    <>
      {dataSave.data.income > 0 && (
        <div className="flex flex-col gap-4">
          <div className="flex w-full flex-col gap-8 bg-slate-50 px-4 py-10 items-center rounded-lg  shadow-sm ">
            <h2 className="text-greenM  font-semibold text-3xl">Resultado</h2>
            <h3 className="text-4xl text-greenM font-bold">{percent}%</h3>
            <p className="text-sm">{tip}</p>
            <a href="/educacion"><button className="rounded-lg p-4 bg-greenM text-white font-medium">
              Mas tips
            </button></a>
          </div>
          <div className="flex w-full flex-col gap-8 bg-slate-50 px-4 py-10 items-center rounded-lg  shadow-sm ">
            ...
            <p className="-mt-8">
              {dataSave.data.expense} / {dataSave.data.income}{" "}
            </p>
          </div>
        </div>
      )}
